...
async function processMessage(bot, msg) {
  ...
  if (!storage.hasChat(chatId) && chatId !== config.adminId) {
    ...
    if (msg.chat.type === 'private' && userId !== config.adminId) {
        ...
        // 2. Если это не команда /start — отшиваем и не тратим токены
        if (command !== '/start') {
        // Имитируем "печатает" пару секунд для реализма
        bot.sendChatAction(chatId, 'typing', getActionOptions(threadId)).catch(() => {});
        
        await new Promise(r => setTimeout(r, 1500)); // Пауза 1.5 сек
        await bot.sendMessage(chatId, "Сорян, я не общаюсь в личке, спроси меня в общем чате, там точно отвечу", getBaseOptions(threadId));
        
        return; // <--- ВАЖНО: Дальше код не идет, нейронка не отвечает
        }
    }
